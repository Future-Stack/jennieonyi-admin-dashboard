...
interface ConfirmReinstateModalProps {
  vendor: VendorType;
  onClose: () => void;
  onConfirm: () => void;
}

export function ConfirmReinstateModal({ vendor, onClose, onConfirm }: ConfirmReinstateModalProps) {
  return (
    <div 
        ...
        <div className="p-[20px]">
          <div className="bg-[#FFFBEB] rounded-[14px] p-4 flex items-start gap-3 mb-5">
            ...
            <p className="text-[14px] text-[#92400E] leading-relaxed">
              Are you sure you want to <span className="font-bold">Reinstate<br/>{vendor.name.split(' ')[0]} {vendor.name.split(' ')[1] || ''}?</span>
            </p>
          </div>
          ...
          <div className="flex gap-3">
            ...
            <button 
              onClick={onConfirm}
              className="cursor-pointer flex-1 h-[42px] rounded-[14px] bg-[#10B981] text-white text-[14px] font-bold hover:bg-[#059669] transition-colors"
            >
              Confirm Reinstate
            </button>
          </div>
